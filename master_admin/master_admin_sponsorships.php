...
<body>
	...
	<div>
		<?php
			...
			if (!$check){
				echo "Error acquiring list of sponsorships: " . mysqli_error($link);
				exit();
			}
			
			else if(($row = mysqli_fetch_array($check)) == NULL) echo "<br><p>No sponsorships has been registered yet.</p><br>";
			else {
				echo "<table id='table'>";
				echo "<tr>";
				echo "<th class='th'>Sponsorship ID#</th>";
				echo "<th class='th'>Sponsor</th>";
				echo "<th class='th'>Sponsored Kid</th>";
				echo "<th class='th'>Grade Level Sponsored</th>";
				echo "<th class='th'>School Year Sponsored</th>";
				echo "<th class='th'>Current Total Sponsored</th>";
				echo "</tr>";
			...
			if(array_key_exists('sponsorshipchange', $_POST)){
				if(!mysqli_query($link, 'update sponsorship set GradeLevel="'.mysqli_real_escape_string($link, $_POST['gradelevel']).'", SchoolYear='.mysqli_real_escape_string($link, $_POST['schoolyear']).', CurrentTotal='.mysqli_real_escape_string($link, $_POST['total']).' where sponsorship.Sponsorship_ID='.mysqli_real_escape_string($link, $_POST['sponsorshipid']).'')) echo 'Error in updating sponsorship relation information: '.mysqli_error($link);
					...
					header('Location: master_admin_sponsorships.php');
				}
